...
public class SOAPClient
{
  ...
  */
  ...
  // Login details
  static final String LOGIN_NAME = "tellarytest";
  static final String LOGIN_PASSWORD = "tellarytest";
  ...
  public static void main(String[] args) throws Exception
  {
    ...
    try
    {
      ...
      testAddWorkLog(jiraSoapService, authToken, issueKey);
...
//            testFindIssuesWithTerm(jiraSoapService, authToken, SOAP_AS_A_SEARCH_TERM);
  ...
  private static void testAddWorkLog(JiraSoapService jiraSoapService, String authToken, String issueKey) throws RemoteException, RemotePermissionException {
    RemoteWorklog worklog = new RemoteWorklog();
    worklog.setComment("test worklog");
    worklog.setTimeSpent("48m");
    worklog.setStartDate(Calendar.getInstance());
    jiraSoapService.addWorklogWithNewRemainingEstimate(authToken, issueKey, worklog, "1h");


    worklog = new RemoteWorklog();
    worklog.setComment("test worklog 2");
    worklog.setTimeSpent("15m");
    worklog.setStartDate(Calendar.getInstance());
    jiraSoapService.addWorklogAndAutoAdjustRemainingEstimate(authToken, issueKey, worklog);

    worklog = new RemoteWorklog();
    worklog.setComment("test worklog 2");
    worklog.setTimeSpent("67m");
    worklog.setStartDate(Calendar.getInstance());
    jiraSoapService.addWorklogAndAutoAdjustRemainingEstimate(authToken, issueKey, worklog);
  }
      ...
      throws java.rmi.RemoteException
  {
    ...
    try
    {
      ...
      for (RemoteFilter filter : savedFilters) {
  ...
  private static void printIssueDetails(RemoteIssue issue)
  {
    ...
    for (Method declaredMethod : declaredMethods) {
      if (declaredMethod.getName().startsWith("get") && declaredMethod.getParameterTypes().length == 0) {
        ...
        try {
          Object obj = declaredMethod.invoke(issue);
